...
class TestChatroomAiOperations(TransactionCase):
    ...
    def test_dashboard_refresh_and_actions(self):
        ...
        self.assertIsInstance(dashboard.open_opportunities, int)
        self.assertGreaterEqual(dashboard.pipeline_value, 0)
        self.assertGreaterEqual(dashboard.stagnant_capital, 0)
        self.assertIsInstance(dashboard.overdue_activities, int)
        self.assertIsInstance(dashboard.today_activities, int)
        self.assertIsInstance(dashboard.ai_requests_today, int)
        self.assertGreaterEqual(dashboard.ai_tokens_today, 0)
        self.assertGreaterEqual(dashboard.ai_failed_today, 0)
        self.assertIsInstance(dashboard.attention_count, int)
        self.assertIn(dashboard.priority_level, ('ok', 'info', 'warning', 'critical'))
        self.assertTrue(dashboard.next_action)
        self.assertTrue(dashboard.setup_next_step)
        self.assertIn(dashboard.ai_config_state, ('disabled', 'incomplete', 'ready'))
        self.assertIn(dashboard.whatsapp_state, ('unavailable', 'incomplete', 'ready'))
        self.assertIn(dashboard.payphone_state, ('unavailable', 'incomplete', 'ready'))
        ...
        self.assertEqual(dashboard.action_open_open_opportunities()['target'], 'new')
        self.assertEqual(dashboard.action_open_overdue_activities()['target'], 'new')
        self.assertEqual(dashboard.action_open_today_activities()['target'], 'new')
        self.assertEqual(dashboard.action_open_ai_usage_today()['target'], 'new')
        self.assertEqual(dashboard.action_open_ai_errors_today()['target'], 'new')
        priority_action = dashboard.action_open_priority()
        self.assertIn(priority_action['type'], ('ir.actions.act_window', 'ir.actions.client'))
        setup_action = dashboard.action_open_setup()
        self.assertIn(setup_action['type'], ('ir.actions.act_window', 'ir.actions.client'))
