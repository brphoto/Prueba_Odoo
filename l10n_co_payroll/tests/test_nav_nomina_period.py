...
class TestNavNominaPeriod(TransactionCase):
    def setUp(self):
        super().setUp()
    ...
    def _create_period(self, date_from="2099-04-01", date_to="2099-04-30"):
        return self.env["nav.nomina.period"].create({
    ...
    def test_csv_export_and_report_are_available(self):
        ...
        report = self.env.ref("l10n_co_payroll.action_report_nav_nomina_period")
    ...
    def test_double_approval_and_novelty_configuration(self):
        self.env["nav.nomina.parameter"].create({
            "company_id": self.company.id,
            "year": 2099,
        ...
        novelty = self.env["nav.nomina.novelty"].create({
            "period_id": period.id,
            "employee_id": self.employee.id,
            "novelty_type": "vac",
        ...
        manager_group = self.env.ref("l10n_co_payroll.group_nav_nomina_manager")
    ...
    def test_operational_records_are_read_only_for_auditor(self):
        auditor_group = self.env.ref("l10n_co_payroll.group_nav_nomina_auditor")
        ...
        self.assertFalse(self.env["nav.nomina.period"].with_user(auditor).has_access("write"))
        self.assertFalse(self.env["nav.nomina.period.line"].with_user(auditor).has_access("create"))
        self.assertFalse(self.env["nav.nomina.period.issue"].with_user(auditor).has_access("unlink"))
    ...
    def test_legal_parameter_versions_select_by_period_date(self):
        ...
        version_two = self.env["nav.nomina.parameter"].create({
            "company_id": self.company.id,
            "year": 2097,
            "version": 2,
    ...
    def test_social_profile_pila_adjustment_and_settlement(self):
        ...
        social = self.env["nav.nomina.social"].create({
            "employee_id": self.employee.id,
        ...
        adjustment = self.env["nav.nomina.adjustment"].create({
            "company_id": self.company.id,
            "employee_id": self.employee.id,
        ...
        config = self.env["nav.nomina.pila.config"].create({"company_id": self.company.id, "name": "PILA prueba", "include_header": True})
        pila_file = self.env["nav.nomina.pila.file"].create({"company_id": self.company.id, "period_id": period.id, "config_id": config.id})
        pila_file.action_generate()
        self.assertEqual(pila_file.state, "generated")
        self.assertTrue(pila_file.attachment_id)
        settlement = self.env["nav.nomina.settlement"].create({
            "company_id": self.company.id,
            "employee_id": self.employee.id,
            "base_salary": 3000,
            "pending_wages": 100,
