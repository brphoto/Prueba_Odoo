...
class NavNominaPilaConfig(models.Model):
    _name = "nav.nomina.pila.config"
...
class NavNominaPilaFile(models.Model):
    _name = "nav.nomina.pila.file"
    ...
    company_id = fields.Many2one("res.company", required=True, default=lambda self: self.env.company, index=True)
    period_id = fields.Many2one("nav.nomina.period", required=True, ondelete="restrict", index=True, tracking=True)
    config_id = fields.Many2one("nav.nomina.pila.config", string="Configuración", required=True, domain="[('company_id', '=', company_id), ('active', '=', True)]")
    ...
    def action_generate(self):
        for record in self:
            ...
            self.env["nav.nomina.audit"].sudo().create({"company_id": record.company_id.id, "period_id": record.period_id.id, "res_model": record._name, "res_id": record.id, "action": "pila", "description": _("Archivo PILA configurable generado: %s.") % filename})
        return True
