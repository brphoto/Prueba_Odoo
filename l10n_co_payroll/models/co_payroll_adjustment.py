...
class CoPayrollAdjustment(models.Model):
    _name = "l10n.co.payroll.adjustment"
    ...
    period_id = fields.Many2one("l10n.co.payroll.period", string="Periodo destino", ondelete="restrict", index=True)
    ...
    def action_apply(self):
        for record in self:
            ...
            if record.period_id:
                self.env["l10n.co.payroll.audit"].sudo().create({"company_id": record.company_id.id, "period_id": record.period_id.id, "res_model": record._name, "res_id": record.id, "action": "other", "description": _("Ajuste %s aplicado: %s.") % (record.adjustment_type, record.amount)})
        return True
