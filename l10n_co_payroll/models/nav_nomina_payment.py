...
class NavNominaPaymentBatch(models.Model):
    _name = "nav.nomina.payment.batch"
    ...
    period_id = fields.Many2one("nav.nomina.period", required=True, ondelete="restrict", index=True)
    ...
    line_ids = fields.One2many("nav.nomina.payment.line", "batch_id", string="Beneficiarios", copy=False)
    ...
    def action_prepare(self):
        for batch in self:
            ...
            values = []
            for period_line in batch.period_id.line_ids.filtered(lambda line: line.net_wage > 0):
                ...
                values.append({"batch_id": batch.id, "period_line_id": period_line.id, "employee_id": period_line.employee_id.id, "bank_account_id": bank.id if bank else False, "amount": period_line.net_wage, "state": "pending" if bank else "error", "error_message": False if bank else _("No hay cuenta bancaria configurada.")})
            self.env["nav.nomina.payment.line"].sudo().create(values)
    ...
    def action_export_csv(self):
        for batch in self:
            ...
            self.env["nav.nomina.audit"].sudo().create({"company_id": batch.company_id.id, "period_id": batch.period_id.id, "res_model": batch._name, "res_id": batch.id, "action": "payment", "description": _("Archivo bancario generado.")})
...
class NavNominaPaymentLine(models.Model):
    _name = "nav.nomina.payment.line"
    _description = "Beneficiario de pago de nómina"
    _order = "employee_id"
    _check_company_auto = True

    batch_id = fields.Many2one("nav.nomina.payment.batch", required=True, ondelete="cascade", index=True)
    company_id = fields.Many2one(related="batch_id.company_id", store=True, readonly=True)
    period_line_id = fields.Many2one("nav.nomina.period.line", required=True, readonly=True)
