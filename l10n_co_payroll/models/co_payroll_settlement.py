...
class CoPayrollSettlement(models.Model):
    _name = "l10n.co.payroll.settlement"
    ...
    transport_allowance = fields.Monetary(string="Auxilio transporte", currency_field="currency_id", help="Se incluye en cesantías y prima cuando corresponda según la parametrización legal.")
    parameter_id = fields.Many2one("l10n.co.payroll.parameter", string="Parámetros legales", domain="[('company_id', '=', company_id)]")
    ...
    def action_calculate(self):
        for record in self:
            ...
            if record.service_days:
                parameter = record.parameter_id or self.env["l10n.co.payroll.parameter"].search([
    ...
    def action_approve(self):
        ...
        for record in self:
            ...
            self.env["l10n.co.payroll.audit"].sudo().create({"company_id": record.company_id.id, "period_id": record.period_id.id if record.period_id else False, "res_model": record._name, "res_id": record.id, "action": "settlement", "description": _("Liquidación aprobada por %s.") % self.env.user.name})
        return True
