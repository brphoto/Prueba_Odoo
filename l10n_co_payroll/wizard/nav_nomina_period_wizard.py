...
class NavNominaPeriodWizard(models.TransientModel):
    _name = "nav.nomina.period.wizard"
    ...
    def action_create_period(self):
        self.ensure_one()
        period = self.env["nav.nomina.period"].create({
        ...
        return {"type": "ir.actions.act_window", "name": _("Periodo de nómina"), "res_model": "nav.nomina.period", "view_mode": "form", "res_id": period.id, "target": "current"}
